...
/**
 * Componente público de barra de filtros para propiedades
 * @param {Object} props
 ...
 * @param {function} props.onPropertiesChange - Callback cuando cambian las propiedades
 */
const PublicPropertyFiltersBar = ({
  filterType = "all",
  onFiltersChange,
  onPropertiesChange,
  initialFilters = {},
  ...
  const [lastSearchFilters, setLastSearchFilters] = useState(initialFilters);
  ...
  // Update lastSearchFilters when initialFilters change
  useEffect(() => {
  ...
  const {
    ...
    getActiveFilters,
    searchWithFilters,
  } = usePublicPropertyFilters(filterType, initialFilters);
  ...
  const notifyFiltersChange = (activeFilters, results = null) => {
    ...
    if (onPropertiesChange && results) {
      onPropertiesChange(results);
    }
  };
  ...
  const handleRemoveFilter = async (filterKey) => {
    ...
    if (Object.keys(updatedFilters).length > 0) {
      sessionStorage.setItem("searchFilters", JSON.stringify(updatedFilters));
    } else {
...
export default PublicPropertyFiltersBar;
