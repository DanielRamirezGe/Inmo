...
export const propertyService = {
   ...
   *
   * @param {Object} axiosInstance - Axios instance with authentication
   ...
   */
  async fetchPropertyDetails(axiosInstance, id, isAdmin) {
    let property = null;
    ...
    try {
      ...
      const propertyData = isAdmin
        ? await api.getPropertyPreview(axiosInstance, id)
        : await api.getPublicPropertyView(axiosInstance, id);
      ...
      if (propertyData?.developmentId || true) {
        const developmentPropertiesResponse = isAdmin
          ? await api.getPublishedProperties(axiosInstance)
          : await api.getPublicProperties(axiosInstance);
   ...
   */
  async submitContactForm(axiosInstance, userData) {
    return await api.submitUserInformation(axiosInstance, userData);
  },
};
